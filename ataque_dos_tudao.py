...
# =================== SELEÇÃO DOS SENSORES PARA ATAQUE DoS ===================
...
# Cria matriz Gamma_y 6x5 fixa (como no artigo), ativando os sensores selecionados
Gamma_y = np.zeros((6, 5))
for idx in selecionados:
    if 0 <= idx <= 4:
        Gamma_y[idx, idx] = 1
    else:
        print(f"⚠️ Índice inválido ignorado: {idx}")
...
def ataque_DoS():
    global estado_anterior, dados_salvos_antes_do_ataque, y_tau_y #variáveis do ataque DoS
    global soc_ataque, kWh_bateria, kW_bateria, kVA_bateria, tensao_bateria, corrente_bateria, Gamma_y #variáveis dos sensores
    global soc_max, soc_min #variáveis dos atuadores 
    ...
    if resultado == 1 and estado_anterior == 0: # salva o valor do sensor
        y_tau_y = np.array([
        [soc_ataque],
        [kWh_bateria],
        [kW_bateria[0]],
    ...
    resultado_do_ataque = y_til_k
    #print(f"resultado do ataque: {resultado_do_ataque[0][0]}")

    soc_ataque_lista.append(float(resultado_do_ataque[0])) 
    kWh_ataque_lista.append(float(resultado_do_ataque[1]))
    kW_ataque_lista.append(float(resultado_do_ataque[2]))
    tensao_ataque_lista.append(float(resultado_do_ataque[3]))
    corrente_ataque_lista.append(float(resultado_do_ataque[4]))
    kVA_ataque_lista.append(float(resultado_do_ataque[5]))

    return resultado_do_ataque        
...
def controle_soc_por_ciclo(stepNumber):
    ...
    # kWh_ataque_lista.append(float(kWh_bateria))
    # kW_ataque_lista.append(float(kW_bateria))
    # tensao_ataque_lista.append(float(tensao_bateria[0]))
    # corrente_ataque_lista.append(float(corrente_bateria[0]))
    # kVA_ataque_lista.append(float(kVA_bateria))
