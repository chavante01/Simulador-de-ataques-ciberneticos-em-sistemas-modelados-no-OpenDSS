...
if modo == "1" or modo == "5":
    ...
    if np.any(Gamma_u[1, :]):
        ...
        c = float(input("  -> Limite inferior: "))
        d = float(input("  -> Limite superior: "))
        #bku[1, 0] = random.uniform(a, b)

    if np.any(Gamma_u[2, :]):
        print("\nCorrupção para kW_ref da Bateria:")
        e = float(input("  -> Limite inferior: "))
        f = float(input("  -> Limite superior: "))
        #bku[2, 0] = random.uniform(a, b)

    if np.any(Gamma_u[3, :]):
        print("\nCorrupção para Idling_kW da Bateria:")
        g = float(input("  -> Limite inferior: "))
        h = float(input("  -> Limite superior: "))
    ...
    if np.any(Gamma_y[0, :]):
        ...
        i = float(input("  -> Limite inferior: "))
        j = float(input("  -> Limite superior: "))
        #bky[0, 0] = random.uniform(a, b)

    if np.any(Gamma_y[1, :]):
        print("\nCorrupção para Energia da Bateria (kWh):")
        k = float(input("  -> Limite inferior: "))
        l = float(input("  -> Limite superior: "))
        #bky[1, 0] = random.uniform(a, b)

    if np.any(Gamma_y[2, :]):
        print("\nCorrupção para Potência Ativa da Bateria (kW):")
        m = float(input("  -> Limite inferior: "))
        n = float(input("  -> Limite superior: "))
        #bky[2, 0] = random.uniform(a, b)

    if np.any(Gamma_y[3, :]):
        print("\nCorrupção para Tensão da Bateria (V):")
        o = float(input("  -> Limite inferior: "))
        p = float(input("  -> Limite superior: "))
        #bky[3, 0] = random.uniform(a, b)

    if np.any(Gamma_y[4, :]):
        print("\nCorrupção para Corrente da Bateria (A):")
        q = float(input("  -> Limite inferior: "))
        r = float(input("  -> Limite superior: "))
        #bky[4, 0] = random.uniform(a, b)

    if np.any(Gamma_y[5, :]):
        print("\nCorrupção para Potência Aparente da Bateria (kVA):")
        s = float(input("  -> Limite inferior: "))
        t = float(input("  -> Limite superior: "))
...
def plotar_resultados_ataques(horas):
    ...
    if enable_false_data_customizable and sensor_soc:
        plt.figure()
        plt.plot(horas, soc_false_injection_customizavel_lista_sensores, label="SOC Sensor False Data Customizable")
    ...
    if enable_false_data_customizable and sensor_v:
        plt.figure()
        plt.plot(horas, tensao_false_injection_customizavel_lista_sensores, label="Tensão Sensor False Data Customizable")
        plot_attack_regions(time_attack_false_data_customizable, step_size)
    ...
    if enable_false_data_customizable and atuador_soc_max:
        plt.figure()
        plt.plot(horas, soc_max_false_injection_customizavel_lista_atuadores, label="SOC Máx Atuador False Data Customizable")
        plot_attack_regions(time_attack_false_data_customizable, step_size)
...
def false_data(): 
    ...
    # Para gerar os valores aleatórios da matriz bku a cada passo de tempo
    bku[0, 0] = random.uniform(a, b)
    bku[1, 0] = random.uniform(c, d)
    bku[2, 0] = random.uniform(e, f)
    bku[3, 0] = random.uniform(g, h)

    # Para gerar os valores aleatórios da matriz bky a cada passo de tempo
    bky[0, 0] = random.uniform(i, j)
    bky[1, 0] = random.uniform(k, l)
    bky[2, 0] = random.uniform(m, n)
    bky[3, 0] = random.uniform(o, p)
    bky[4, 0] = random.uniform(q, r)
    bky[5, 0] = random.uniform(s, t)
    ...
    # Plotagem do intervalo de ataque de injeção de dados falsos
    intervalos_false_data.append(x)
    time_attack_false_data.append(1)  # Sempre ativo se chamado

    # if np.any(np.dot(Gamma_y, bky) != 0) or np.any(np.dot(Gamma_u, bku) != 0):
    ...
    # Funciona chamar os elementos assim também:
    # soc_false_injection_lista_sensores.append(resultado_ataque_false_data_2[0,0])
...
def injecao_dados_falsos_customizavel():
    ...
    if 20 <= x <= 60: #intervalo de ataque    
        ...
        intervalos_false_data_customizable.append(x)
        time_attack_false_data_customizable.append(1)

    else: 
        u_til_k = uk
        y_til_k = yk
        time_attack_false_data_customizable.append(0)

    soc_false_injection_customizavel_lista_sensores.append(float(y_til_k[0].item()))
    kWh_false_injection_customizavel_lista_sensores.append(float(y_til_k[1].item()))
    kW_false_injection_customizavel_lista_sensores.append(float(y_til_k[2].item()))
    tensao_false_injection_customizavel_lista_sensores.append(float(y_til_k[3].item()))
    corrente_false_injection_customizavel_lista_sensores.append(float(y_til_k[4].item()))
    kVA_false_injection_customizavel_lista_sensores.append(float(y_til_k[5].item()))
    soc_max_false_injection_customizavel_lista_atuadores.append(float(u_til_k[0].item())) 
    soc_min_false_injection_customizavel_lista_atuadores.append(float(u_til_k[1].item()))
    kW_ref_false_injection_lista_customizavel_atuadores.append(float(u_til_k[2].item()))
    idling_kW_false_injection_customizavel_lista_atuadores.append(float(u_til_k[3].item()))

    return u_til_k, y_til_k
